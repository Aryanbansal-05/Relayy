...
// Assuming 'relayyLogo' and 'useAuth' are correctly defined and imported
...
  // This handler is used by both desktop form submission and the mobile search bar component.
...
  // --- Original Logic (Handle Logout, Fetch Unread Count, etc.) ---
...
  // --- Main Navbar Structure (Desktop View) ---
...
          <Link to="/contact" className={`relative text-gray-700 hover:text-emerald-700 transition
...
            Contact
          </Link>
          <Link to="/about" className={`relative text-gray-700 hover:text-emerald-700 transition
...
          
          {/* 1. Desktop Search Bar (Large Screen Only) */}
          {!hideSearch && (
...
          )}
          
          {/* List an Item (Desktop/Tablet Only) */}
          <Link
            to="/sell"
            className="hidden md:flex items-center bg-gradient-to-r from-emerald-600 to-emerald-700 text-white px-5 py-2.5 rounded-full font-semibold hover:from-emerald-700 hover:to-emerald-800 transition shadow-md whitespace-nowrap"
          >
...
            {/* REMOVED: Redundant Mobile Search Icon Button */}
            
            <button
...
      </nav>
      
      {/* Dedicated Mobile Search Bar (appears below the header) */}
      {!hideSearch && (
        <MobileSearchBar 
...
// --- New Component for Mobile Search Bar ---
...
                    {/* Search icon inside the input bar */}
                    <Search className="w-5 h-5 text-gray-500 mr-2" />
                    <input
                        type="text"
                        placeholder="Search products..."
...
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="flex-1 outline-none bg-transparent text-gray-800 placeholder-gray-500 text-base"
                        // Pressing 'Enter' submits the form, triggering handleSearchSubmit
                    />
                    {/* Optionally: A visible submit button for mobile, though 'Enter' usually suffices */}
...
// --- Bottom Navbar Component (Simplified props) ---

function BottomNavbar({ isActive, unreadCount }) {
    // NOTE: Removed unused props: hideSearch, searchQuery, setSearchQuery
    return (
...
            
            {/* Home */}
            <Link to="/home" className={`flex flex-col items-center justify-center p-2.5 flex-1 transition-colors ${
                isActive("/home") ? "text-emerald-600" : "text-gray-500 hover:text-emerald-500"
            }`}>
...
            {/* Browse */}
            <Link to="/all-products" className={`flex flex-col items-center justify-center p-2.5 flex-1 transition-colors ${
                isActive("/all-products") || isActive("/all-products?q") ? "text-emerald-600" : "text-gray-500 hover:text-emerald-500"
            }`}>
                <LayoutList className="w-6 h-6" />
                <span className="text-xs font-medium">Browse</span>
            </Link>

            {/* Sell/List Item (Center CTA) */}
...
            {/* Messages */}
            <Link to="/inbox" className={`flex flex-col items-center justify-center p-2.5 relative flex-1 transition-colors ${
                isActive("/inbox") ? "text-emerald-600" : "text-gray-500 hover:text-emerald-500"
            }`}>
...
            {/* Profile */}
            <Link to="/profile" className={`flex flex-col items-center justify-center p-2.5 flex-1 transition-colors ${
                isActive("/profile") ? "text-emerald-600" : "text-gray-500 hover:text-emerald-500"
            }`}>
                <User className="w-6 h-6" />
